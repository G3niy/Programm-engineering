...
from serv_redis.cache_redis import RedisCache
...
async def clients_services(service_id: str, token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(get_db), redis_service: RedisCache = Depends()):
    ...
    result_redis = await redis_service.get_cached_task(service_id)
    if result_redis is not None:
        return result_redis
    result = await db.execute(select(Services).filter(Services.code == service_id))
    task = result.scalars().first()
    if task is None:
        raise HTTPException(status_code=404, detail="Проект не найден")
    await redis_service.cache_task(task.as_dict())
    return task
